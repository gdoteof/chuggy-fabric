...
process identically -- and only the first reads like a mistake. Each is checked
here; neither is checked anywhere else.
...
# What replaced it, and its two numbers. The schema is strict in both
# directions, so an absent `lead` refuses the process exactly as a present
# `policy` does -- and the absence is the easier of the two to leave behind,
# because deleting a block is one edit and writing its replacement is another.
...
def main():
    ...
    # 2b. And what replaced it is there. The two are one swap, and only the first
    #     half of it fails loudly on its own: a leftover `policy` block is caught
    #     above, while a missing `lead` block reads as a document that simply
    #     does not mention the lead. The process refuses both identically, so
    #     this file does too.
    lead = document.get(LEAD_CONFIGURATION_KEY)
    if not isinstance(lead, dict):
        refuse(
            f"the configuration carries no {LEAD_CONFIGURATION_KEY!r} block; the image that "
            "retires the trusted policy protocol requires one, and its schema is strict"
        )

...
pkgs.runCommand "chuggy-configuration-importer" {
  ...
  test "$(grep -c 'commit=\"$(git ls-remote' "$manifest")" -eq 1
  ...
  grep -F 'git ls-remote --exit-code "$repository" refs/heads/main >/dev/null 2>&1 && break' "$manifest" >/dev/null
  test "$(grep -c 'for i in \$(seq 1 30)' "$manifest")" -eq 2
